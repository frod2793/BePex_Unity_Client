...
### 2.1 어드레서블 및 서버 연동 파이프라인 개요
본 프로젝트는 에디터 시점의 정적 에셋 임베딩에 국한되지 않고, 실시간 라이브 서비스 갱신을 위한 동적 데이터 패치 파이프라인을 구축해 두었습니다.

1. 원격 서버 배포 (어드민):
   - 기획자가 EventAdminScene에서 이벤트를 편집한 후 [Firebase 서버 배포]를 실행하면, **MockFirebaseUploadService(서버 배포 시뮬레이터)**를 통해 가상 전송 업로드(1.5초 비동기 지연 모킹) 과정을 시뮬레이션 검증하여 배포 성공 유무를 사전에 진단합니다.
2. 어드레서블 런타임 핫패치 (인게임):
   - Inagme 클라이언트 기동 시, Composition Root인 EventSceneInitializer가 UpdateAddressableCatalogAsync()를 호출하여 원격 어드레서블 카탈로그의 갱신 정보를 비동기로 탐색합니다.
   - 변경 사항이 식별될 경우 원격 서버 CDN(AWS S3 등)으로부터 업데이트된 에셋 번들을 런타임에 동적으로 사전 다운로드하여 최신 상태를 복원합니다.
   - 네트워크 연결 실패 및 CDN 오류 등 비정상 상황 발생 시, try-catch 예외 가드를 거쳐 안전하게 디스크 로컬 캐시 또는 기기 내장 번들로 폴백(Fallback)되어 크래시 없이 실행됩니다.
...
#### 예시 코드 (Pure C# 및 의존성 주입)

```csharp
// 1. StageManager 등 인게임 시스템에서 EventPointManager를 생성자나 프로퍼티로 주입받습니다.
public class StageManager
{
    private readonly EventPointManager m_pointManager;

    public StageManager(EventPointManager pointManager)
    {
        m_pointManager = pointManager;
    }

    // 2. 스테이지 클리어 등 이벤트 발생 시 포인트를 적립합니다. (하드코딩 키 노출 없음)
    public void OnStageClear()
    {
        m_pointManager.AddPoint(15);
    }
}
```
(예시: `StageManager`에 주입된 `EventPointManager` 인스턴스의 `AddPoint` 단 1줄만 호출하면 내부 데이터 흐름이 자동으로 수행됩니다.)
