...
EventTableSO / EventDefinitionSO / ConditionDefinitionSO / RewardDefinitionSO / SeasonPassDefinitionSO / ConditionTypeSO / RewardTypeSO / ConditionTypeRegistrySO / RewardTypeRegistrySO: 데이터 정의 및 에디터 직렬화, Type Object를 위한 스크립터블 오브젝트.
...
QuestConditionAttribute: 조건 클래스와 ConditionTypeSO의 식별자(TypeName) 문자열을 런타임에 동적으로 매핑시키는 커스텀 특성.
...
QuestRewardAttribute: RewardTypeSO의 식별자(TypeName) 문자열과 런타임 구현 클래스를 연결해주는 커스텀 어트리뷰트.
...
Assets/_Game/Data/ConditionTypes/GuildEvent.asset 에셋이 자동 생성 및 ConditionTypeRegistry에 자동 등록되며, Assets/_Game/Scripts/EventSystem/Conditions/GuildEventQuestCondition.cs가 자동 생성되면 세부 로직만 구현합니다.

[방법 B] 수동 수정 방법

① Assets/_Game/Data/ConditionTypes/ 폴더 내에 마우스 우클릭 -> Create -> BePex -> Condition Type 메뉴를 통해 GuildEvent.asset 에셋을 생성합니다. (인스펙터에서 Type Name을 GuildEvent, Display Name을 길드 이벤트로 설정)
② 생성한 에셋을 Assets/_Game/Data/ConditionTypeRegistry.asset의 Conditions 리스트에 수동으로 추가합니다.
...
Assets/_Game/Data/RewardTypes/SeasonPoint.asset 에셋이 자동 생성 및 RewardTypeRegistry에 자동 등록되며, Assets/_Game/Scripts/EventSystem/Rewards/SeasonPointQuestReward.cs가 자동 생성되면 Grant 메서드에 재화 가산 로직을 작성합니다.

[방법 B] 수동 수정 방법

① Assets/_Game/Data/RewardTypes/ 폴더 내에 마우스 우클릭 -> Create -> BePex -> Reward Type 메뉴를 통해 SeasonPoint.asset 에셋을 생성합니다. (인스펙터에서 Type Name을 SeasonPoint, Display Name을 시즌 포인트로 설정)
② 생성한 에셋을 Assets/_Game/Data/RewardTypeRegistry.asset의 Rewards 리스트에 수동으로 추가합니다.
...
public async Awaitable AddAdWatchPointAsync(int amount, ISaveSystem saveSystem, PlayerRewardModel playerReward)
{
    if (playerReward != null)
    {
        playerReward.AddCurrency("Point", amount);
