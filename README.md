...
1. **원격 서버 배포 (어드민)**:
   - 기획자가 `EventAdminScene`에서 이벤트를 편집한 후 [Firebase 서버 배포]를 실행하면, `MockFirebaseUploadService`를 통해 중앙 데이터베이스에 실시간으로 최신 JSON 데이터 테이블이 업로드(1.5초 비동기 지연 모킹)되어 배포 준비를 마칩니다.
2. **어드레서블 런타임 핫패치 (인게임)**:
   - `Inagme` 클라이언트 기동 시, Composition Root인 `EventSceneInitializer`가 `UpdateAddressableCatalogAsync()`를 호출하여 원격 어드레서블 카탈로그의 갱신 정보를 비동기로 탐색합니다.
