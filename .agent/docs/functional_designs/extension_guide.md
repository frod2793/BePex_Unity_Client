...
3. **Fallback-driven Factory (폴백 기반 팩토리)**: 어셈블리 내에 매핑되는 C# 클래스 소스 파일이 없더라도, 범용 전략 클래스(`StandardQuestCondition`, `GeneralQuestReward`)로 자동 대체(Fallback)하여 동작하게 함으로써 C# 컴파일 및 재빌드 없는 확장을 지원합니다.

이로 인해 새로운 조건이나 보상이 추가되어도 **기존 팩토리 클래스(`QuestConditionFactory`, `QuestRewardFactory`)의 소스 코드는 전혀 수정할 필요가 없습니다.**

---

## 1.2 코드 작성 없는 데이터 기반 확장 (OCP 극대화)

본 이벤트 시스템의 가장 강력한 특징은 **단순 누적값 비교 조건**이나 **기본 재화 지급 보상**의 경우, 개발자가 **C# 소스 코드 파일을 전혀 만들지 않고** 기획자 혼자서 데이터 세팅만으로 확장이 가능하다는 점입니다.

- **원리**: 팩토리에서 기획 데이터의 `typeName`에 해당하는 C# 구현체 클래스를 찾지 못할 경우, 자동으로 범용 클래스인 `StandardQuestCondition`과 `GeneralQuestReward`를 인스턴싱하여 기획자가 지정한 `typeName` 키값과 데이터를 주입해 연동합니다.
- **적용 대상**:
  - **조건(Condition)**: 특정 수치가 타겟값에 도달했는지 여부만 단순 비교하는 모든 누적형 퀘스트 조건 (예: 몬스터 처치, 스테이지 클리어, 광고 시청 등).
  - **보상(Reward)**: `PlayerRewardModel` 딕셔너리에 문자열 키로 적립할 수 있는 모든 보상 (예: 경험치, 재화, 뽑기 티켓, 포인트 등).
...
2. 실행된 창에서 아래 항목들을 입력합니다:
   ...
   - **C# 클래스 파일 추가 생성 여부**: 
     - **[체크 해제 - 권장]**: 단순 값 비교 조건이나 기본 재화 보상의 경우, 체크를 해제하여 C# 소스 코드 생성 없이 에셋 데이터만으로 즉시 연동을 수행합니다 (OCP 극대화).
     - **[체크 선택]**: 고유한 시간 비교나 날짜 가드 등 특수한 런타임 제어 로직이 필요하여 C# 전략 스크립트를 구현해야 할 때만 체크하여 템플릿을 생성합니다.
...
- **클래스 템플릿 자동 생성** (C# 클래스 추가 생성 체크 시):
...
1. `ConditionDefinitionSO` 에셋을 생성하고, `Condition Type` 슬롯에 새롭게 추가하거나 등록된 `GuildEvent.asset` 에셋을 할당합니다.
...
- **[주의]**: 씬에 구성된 `[System]/EventAdminSceneInitializer` 컴포넌트의 `m_conditionTypeRegistry` 직렬화 필드에 `Assets/_Game/Data/ConditionTypeRegistry.asset` 에셋이 정상 할당되어 있어야 합니다.
...
- **[주의]**: 씬에 구성된 `[System]/EventAdminSceneInitializer` 컴포넌트의 `m_rewardTypeRegistry` 직렬화 필드에 `Assets/_Game/Data/RewardTypeRegistry.asset` 에셋이 정상 할당되어 있어야 합니다.
